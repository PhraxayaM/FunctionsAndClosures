...
let nameSorting = guestsToSort.sorted { (guestA, guestB) -> Bool in
    return guestA.name < guestB.name
}

//sort the guests by age, but in descending order (youngest at the front of the array)

let guestsYoungestToOldest = guestsToSort.sorted { (guestA, guestB) -> Bool in
...
let evenNumbers = numbersToFilter.filter { (num) -> Bool in
    return num % 2 == 0
}
